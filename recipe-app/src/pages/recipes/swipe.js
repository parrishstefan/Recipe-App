...
Swipe.getInitialProps = async (ctx) => {
  ...
  var unformatted_allergies = "";
  var isEmpty = "good"

  if (allergies) {
    allergies.map((str) => {
      unformatted_allergies += str + ",";
    });
  }
  if (diet) {
    diet.map((str) => {
  ...
  var formatted_allergies = unformatted_allergies.toLowerCase().slice(0, -1);
  var formatted_diet = unformatted_diet.toLowerCase().slice(0, -1);
  if (!formatted_allergies && !formatted_diet) {
    isEmpty = "none";
  }
